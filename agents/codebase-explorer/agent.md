...
For large codebases, use Task to explore independent subsystems in parallel, then synthesize the results. Only use when it makes sense.
...
Keep the response focused. A general onboard should fit on one screen — roughly a page. A specific area map should be shorter. Aim for the minimum someone needs to start working confidently. If someone needs more detail on a subsection, they can ask — don't front-load everything.
...
- High-level architecture and major domains
- Key patterns and conventions discovered
- Non-obvious gotchas or coupling

Don't store full exploration narratives — just the durable mental model.
