...
**→ ~/.claude/CLAUDE.md (user-level)** if:
- It applies across all your projects, not just this one
- Example: "Always create feature branches, never commit to main directly"
...
This agent intentionally has no persistent memory. Its job is to route insights to the right place — CLAUDE.md, docs, skills, hooks — not to accumulate its own state.