...
- (Post-commit default — this agent typically runs after changes are committed.)
...
If asked for a **dry run**, skip to Step 5 and return only what you *would* change without making edits.
...
- **Less is more.** A project with 5 accurate, up-to-date doc files is better than 50 stale ones.
 ...
 - Which docs map to which systems
 - Style patterns (terse vs detailed, diagrams vs prose)
 
 Don't store per-session change details.
